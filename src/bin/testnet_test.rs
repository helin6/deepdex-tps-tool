...
async fn main() -> anyhow::Result<()> {
    ...
    let market_ids = vec![TEST_MARKET_ID];
    tokio::time::sleep(Duration::from_millis(5000)).await;

    info!("init wallet account......");
    let mut test_accounts = create_extra_test_accounts(TEST_ACCOUNT_NUM).await?;
    ...
    for (index, id) in market_ids.iter().enumerate() {
        ...
        let test_accounts: &[AccountDetail] = &test_accounts[..];
        {
...
async fn build_batch_ops(
    ...
    for i in 1..=n {
        let op = if MATCHED_PERCENT == 100 {
            debug!("{user_name} subaccount: {subaccount:?} build place order nonce: {nonce} for i: {i}");
            cancel_id += 1;
            let price = target_price(
                user_name,
                is_long,
...
fn target_price(_user_name: &str, is_long: bool, pre_price: u128, match_price: u128, cancel_id: u32, skip_cancel: &mut bool) -> u128 {
